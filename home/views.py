...
def services(request):
    return HttpResponse(request, 'services.html')
def articles(request):
    return HttpResponse(request, 'articles.html')
...
def detect_ai(request):
    ...
    if request.method == 'POST' and request.FILES.get('file'):
        ...
        full_file_path_2 = fs.url(file_path)
        ...
        if file_extension == '.mp3':
            ...
            # print(result_22)
            result = full_file_path, " ", full_file_path_2, " ", result_22
            image_url = "..", full_file_path_2

    return render(request, 'detect.html', {'result': result, 'image_url': image_url})
